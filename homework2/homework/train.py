...
def train(
    ...
    **kwargs,
    ...
    log_dir = _resolve_path(exp_dir) / f"{model_name}_{datetime.now().strftime('%m%d_%H%M%S')}"
    ...
    data_dir = _resolve_path(kwargs.pop("data_dir", "classification_data"))
    train_data = load_data(str(data_dir / "train"), shuffle=True, batch_size=batch_size, num_workers=0)
    val_data = load_data(str(data_dir / "val"), shuffle=False)
